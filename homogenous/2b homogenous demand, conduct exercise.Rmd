...
```{r q_p}
q_p1 <- lm(ln_qu ~ ln_cprice + q1 + q2 + q3, data = coffee)
summary(q_p1)
...
q_p2 <- lm(ln_qu ~ ln_cprice + q1 + q2 + q3 + ln_tprice, data = coffee)
summary(q_p2)
...
q_p3 <- lm(formula = ln_qu ~ ln_cprice + q1 + q2 + q3 + ln_tprice + ln_incom, data = coffee)
summary(q_p3)
...
```{r iv1, echo=FALSE, warning=FALSE}
library("AER")
iv1 <- ivreg(ln_qu ~ q1 + q2 + q3 + ln_cprice | q1 + q2 + q3 + ln_bprice, data= coffee)
summary(iv1)
...
linear1 <- ivreg(qu ~ incom + q1 + q2 + q3 + cprice | bprice + incom + q1 + q2 + q3, data = coffee)
summary(linear1)
