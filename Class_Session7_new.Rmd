...
We consider profits $\pi_i(q_i) = P(Q)\cdot q_i-C_i(q_i)$
...
## Infer normalized conduct $\phi$

Lerner Index is given by $L=\frac{P-MC}{P}=\frac{\phi}{\epsilon_D}$. Solve for $\phi$
...
Back out Lerner markups $L=\frac{P-MC}{P}=\frac{1}{N \cdot \epsilon_D}$

```{r, echo=TRUE}
epsilon_D = -0.28
# Monopoly
L_monopoly=1/abs(epsilon_D)
L_monopoly
# Cournot with N=10?
L_cournot=1/(10*abs(epsilon_D))
L_cournot
```

- we can reject that there is monopoly: 360% markup 
- conduct is assumed to be 0 in Cournot

# Cost information from industry
...
## Implied number of firms

Once we know $\epsilon_D$ and $L$ or $\phi=L*\epsilon_D$, we can back out 

$$N=\frac{1}{L \cdot \epsilon_D}$$

which is implied number of firms as validation for our model 

- check if equal to number of firms in the market

only works for Cournot with simultaneous moves

- no analytical solution for conjectural variations
- intuition: number of firms should be lower with CV
- you can use Cournot as an estimate of the upper bound
...
Ahold Delhaize Coffee Company, Jacobs Douwe Egberts, Peeze, Pelican Rouge Coffee Roasters, Boot Coffee, Espresso Fabriek, Beans Coffee, and Back to Black who operate in the market. Jacobs and Douwe Egberts merged recently
